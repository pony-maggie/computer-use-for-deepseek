...
Computer Use for DeepSeek 更适合作为一个可监督的浏览器和文件助手：它可以帮你准备、检查、整理和起草内容，但最终关键决策仍由你掌控。

适合的场景：

- 网页资料整理：打开公开网页、对比产品、收集链接、总结信息，并把页面内容整理成结构化笔记。
- 表单和后台工具：根据参考资料填写草稿、检查字段、预览改动，并在提交前停下来。
- 文件处理：总结上传文档、提取要点、改写内容，并把生成文件保存到任务工作区。
- 产品 QA：打开网站、走一遍用户流程、检查布局或语言切换，并报告卡住的位置。
- 重复性网页流程：进入熟悉的后台、检查状态、下载报表，或准备好需要你审核的周期性浏览器任务。

不适合全自动执行的场景：

- 未经人工确认就发送邮件、发送消息、发布内容或提交表单。
- 付款、购买、取消订单、修改权限或删除数据。
- 登录、验证码、双因素认证、修改密码或账号恢复。
- 法律、财务、医疗等高风险决策。
- 高频且稳定的 API 自动化；这类任务更适合写专门脚本或集成，而不是用浏览器控制。

## 任务历史

创建任务后，当前 run 面板会持续显示正在执行的任务内容，不会因为输入框清空而丢失上下文。左侧栏也会保留任务历史，包括 run ID、任务内容、状态、时间，以及可用时的最终结果。历史记录存储在 `./start.sh` 使用的本地 `data/` 目录里，重启应用后仍可查看。

## 无感记忆

Computer Use for DeepSeek 包含无感记忆：应用会在后台从已完成任务中保留少量长期有用的经验，让之后的任务不必每次都从零开始。它记住的是你的工作习惯和稳定偏好，不是每次任务的私密细节。

例如，它可以记住你偏好的回复语言、常用工作流、稳定的沙箱环境事实，以及失败后的简短教训。它不应该保存上传文件正文、截图、DOM dump、凭据、一次性网页内容，或逐步点击历史。

![无感记忆架构](docs/images/memory-architecture.png)

这套记忆不会出现在普通 UI 里。只有当未来任务相关时，agent 才会收到一小段隐藏的 `Memory Context`；现有安全策略仍然负责判断哪些动作必须阻止或人工确认。
